...
module Http =
    ...
    /// myGamesPath should point to the user's "My Games" directory.
    ...
    /// Create an API config with the default base URL.
    let createConfig (commKey: string) : ApiConfig =
        { BaseUrl = "http://localhost:31270"
          CommKey = commKey }

    /// Create an API config with a custom base URL.
    let createConfigWithUrl (baseUrl: string) (commKey: string) : ApiConfig =
        { BaseUrl = baseUrl
          CommKey = commKey }

    /// Send an authenticated GET request to the TSW API and deserialize the response.
